...
function EmployeeRecords() {
  ...
  const loadData = async () => {
    setLoading(true)
    const [employeeData, recordsData] = await Promise.all([
      getEmployeeById(id),
  ...
  const handleDelete = async (date) => {
    if (window.confirm('정말 삭제하시겠습니까?')) {
      const updated = await deleteEmployeeRecord(id, date)
      if (updated) {
        setRecords(updated)
      }
    }
  ...
  const handleSaveEdit = async () => {
    const updated = await updateEmployeeRecord(id, editingDate, editForm)
    if (updated) {
      setRecords(updated)
    }
    setEditingDate(null)
    ...
    return (
      <div className="employee-records-page">
        <div className="error-state">
          ...
          <Link to="/admin" className="back-link">목록으로 돌아가기</Link>
        </div>
      </div>
    )
  }
